...
//#include "IRremoteNF.h"
//#include "IRremoteIntNF.h"
...
// To use them, set MYTEST in IRremoteInt.h
...
void mark(int time) {
  ...
  if (time > 0) while(time--) _delay_ms(1);//delayMicroseconds(time);
}
...
int MATCH(int measured, int desired) {
  ...
  USART_putstring(TICKS_LOW(desired) );
  USART_putstring(" <= ");
  USART_putstring(measured);
  USART_putstring(" <= ");
  USART_putstring(TICKS_HIGH(desired) );
  return measured >= TICKS_LOW(desired) && measured <= TICKS_HIGH(desired);
}

int MATCH_MARK(int measured_ticks, int desired_us) {
  USART_putstring("Testing mark ");
  USART_putstring(measured_ticks * USECPERTICK);
  USART_putstring(" vs ");
  USART_putstring(desired_us);
  USART_putstring(": ");
  USART_putstring(TICKS_LOW(desired_us + MARK_EXCESS));
  USART_putstring(" <= ");
  USART_putstring(measured_ticks);
  USART_putstring(" <= ");
  USART_putstring(TICKS_HIGH(desired_us + MARK_EXCESS));
  return measured_ticks >= TICKS_LOW(desired_us + MARK_EXCESS) && measured_ticks <= TICKS_HIGH(desired_us + MARK_EXCESS);
}

int MATCH_SPACE(int measured_ticks, int desired_us) {
  USART_putstring("Testing space ");
  USART_putstring(measured_ticks * USECPERTICK);
  USART_putstring(" vs ");
  USART_putstring(desired_us);
  USART_putstring(": ");
...
long decodeSanyo(decode_results *results) {
  ...
  // Initial space  
  /* Put this back in for debugging - note can't use #MYTEST as if Debug on we don't see the repeat cos of the delay
  USART_putstring("IR Gap: ");
  USART_putstring( results->rawbuf[offset]);
  USART_putstring( "test against:");
  USART_putstring(results->rawbuf[offset]);
  */
...
long decodeMitsubishi(decode_results *results) {
  // USART_putstring("?!? decoding Mitsubishi:");USART_putstring(irparams.rawlen); USART_putstring(" want "); USART_putstring( 2 * MITSUBISHI_BITS + 2);
  long data = 0;
  ...
  */
  ...
  while (offset + 1 < irparams.rawlen) {
    if (MATCH_MARK(results->rawbuf[offset], MITSUBISHI_ONE_MARK)) {
      ...
      // USART_putstring("A"); USART_putstring(offset); USART_putstring(results->rawbuf[offset]);
      return ERR;
    }
    offset++;
    if (!MATCH_SPACE(results->rawbuf[offset], MITSUBISHI_HDR_SPACE)) {
      // USART_putstring("B"); USART_putstring(offset); USART_putstring(results->rawbuf[offset]);
      break;
    }
    offset++;
  }
